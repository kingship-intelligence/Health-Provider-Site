          ...
          <div className="flex flex-col md:flex-row gap-12 items-start">
            <div className="w-full md:w-1/3 max-w-sm shrink-0">
              <div className="aspect-[3/4] rounded-3xl overflow-hidden shadow-lg bg-muted relative">
                {provider.photoUrl ? <img src={provider.photoUrl} alt={provider.name} className="w-full h-full object-cover" /> : null}
              </div>
            </div>
              ...
              <ul className="space-y-4">
                {provider.education.map((edu, i) => (
                  <li key={i} className="flex gap-4 p-4 rounded-2xl bg-card border">
                    <div className="w-10 h-10 rounded-full bg-primary/10 text-primary flex items-center justify-center shrink-0">🎓</div>
