      ...
      <section className="py-24 bg-muted/50">
        <div className="container mx-auto px-4">
          <div className="flex justify-between items-end mb-12">
            <div className="max-w-2xl"><h2 className="text-3xl md:text-4xl font-serif mb-4">Meet our clinicians</h2><p className="text-muted-foreground text-lg">Experienced, compassionate providers who take the time to listen.</p></div>
            <Link href="/providers" className="hidden md:block"><Button variant="ghost">Browse Directory &rarr;</Button></Link>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
            {featuredProviders?.slice(0, 2).map((provider, i) => (
              <motion.div key={provider.id} initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}>
        ...
        <div className="container mx-auto px-4 text-center max-w-4xl">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {testimonials?.slice(0, 2).map((t, i) => (
              <motion.div key={t.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.2 }} className="bg-card p-8 rounded-3xl text-left shadow-sm border">
